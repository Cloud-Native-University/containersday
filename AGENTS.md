...
- **Stale-tab refresh** — GitHub Pages serves the agenda with `cache-control: max-age=600`, and an attendee who opened it in the morning keeps that tab all day, so a schedule change reaches nobody until they reload. Nothing can purge their browser from here, so `list.html` fingerprints the schedule it was built from (session ids, times, rooms — not abstracts, which should not cost a reload) into `/agenda-stamp.json` and embeds the same digest on the page. The page re-checks that file on load, on every return to the tab, and every two minutes, with both a cache-busting query and `cache: no-store` so neither the CDN nor the browser can answer it stale. A changed digest reloads a hidden tab silently and shows a "La agenda cambió" bar on a visible one. It follows that **an attendee still sees the old schedule for up to two minutes, and only if their tab has network** — it is a safety net, not a broadcast channel.
